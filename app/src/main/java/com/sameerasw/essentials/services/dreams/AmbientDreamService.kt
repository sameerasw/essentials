...
class AmbientDreamService : DreamService() {
    ...
    private val progressUpdateRunnable = object : Runnable {
        override fun run() {
            ...
            try {
                ...
                val componentName = android.content.ComponentName(this@AmbientDreamService, com.sameerasw.essentials.services.NotificationListener::class.java)
                ...
                if (activeSession != null) {
                    ...
                    val currentTitle = metadata?.getString(android.media.MediaMetadata.METADATA_KEY_TITLE)
                    val currentArtist = metadata?.getString(android.media.MediaMetadata.METADATA_KEY_ARTIST)
    ...
    private fun setupContentUI(parentInfo: FrameLayout) {
         ...
         volumeStrokeView = VolumeStrokeView(this, petalPath, 0).apply {
             layoutParams = FrameLayout.LayoutParams(size + dpToPx(20f), size + dpToPx(20f), Gravity.CENTER)
             setColor(Color.GRAY)
         }
         ...
         titleView = TextView(this).apply {
             ...
             setTextColor(Color.WHITE)
             gravity = Gravity.CENTER
             layoutParams = LinearLayout.LayoutParams(dpToPx(280f), LinearLayout.LayoutParams.WRAP_CONTENT).apply {
         ...
         artistView = TextView(this).apply {
             ...
             gravity = Gravity.CENTER
              layoutParams = LinearLayout.LayoutParams(dpToPx(240f), LinearLayout.LayoutParams.WRAP_CONTENT).apply {
        ...
        if (volumeReceiver == null) {
            ...
            registerReceiver(volumeReceiver, IntentFilter("android.media.VOLUME_CHANGED_ACTION"), Context.RECEIVER_EXPORTED)
        }
    ...
    private fun checkIsLiked(session: android.media.session.MediaController): Boolean {
         try {
             ...
             for (action in pbState.customActions) {
                 ...
                 if (name.contains("Unlike", true) || name.contains("Unheart", true) || name.contains("Remove", true)) return true
             }
