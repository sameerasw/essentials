...
object ShutUpManager {
    ...
    suspend fun applyShutUpSettings(context: Context, config: ShutUpAppConfig) {
        ...
        withContext(Dispatchers.IO) {
            config.settings.forEach { setting ->
                if (setting.enabled) {
                    safeWriteSetting(context, setting.settingType, setting.key, setting.valueOnLaunch)
                }
            }
        }
    }
    ...
    suspend fun revertShutUpSettings(context: Context, config: ShutUpAppConfig) {
        ...
        withContext(Dispatchers.IO) {
            config.settings.forEach { setting ->
                if (setting.enabled) {
                    safeWriteSetting(context, setting.settingType, setting.key, setting.valueOnRevert)
                }
            }
        }
    }
